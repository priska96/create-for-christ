...
import { apiUrl, authClient } from "./src/auth-client";
import {
  ActivityIndicator,
  Image,
  Pressable,
...
export default function App({
  ...
  async function logout() {
    ...
    try {
      ...
      if (result.error) {
        setAccountError("Abmelden fehlgeschlagen. Bitte erneut versuchen.");
        return;
      }
      router.replace("/");
    } catch {
      setAccountError("Keine Verbindung. Bitte erneut versuchen.");
    } finally {
      setSigningOut(false);
    }
  }
        ...
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.tags}>
            <Pressable
              ...
              onPress={() =>
                router.push(authenticated ? "/profile" : "/sign-up")
              }
              style={styles.button}
            >
            ...
            <Pressable
              ...
              disabled={signingOut}
              onPress={() =>
                authenticated ? void logout() : router.push("/sign-in")
              }
              style={styles.filter}
            >
            ...
            <View style={styles.roles}>
              {(["creator", "brand"] as const).map((value) => (
                ...
                >
                  ...
                  >
                    {value === "creator" ? "Für Creator" : "Für Brands"}
                  </Text>
                </Pressable>
              ))}
            </View>
          ...
          {role === "creator" ? (
            <>
              ...
              {state === "loading" ? (
                <View style={styles.empty}>
                  <ActivityIndicator color="#36584A" />
                ...
                <View style={styles.empty} accessibilityLiveRegion="polite">
                  ...
                  <Text style={styles.body}>
                    Die Kampagnen sind gerade nicht erreichbar. Versuch es
                    gleich noch einmal.
                  </Text>
                  <Pressable
                    accessibilityRole="button"
                ...
                campaigns.map((campaign) => (
                  <View key={campaign.id} style={styles.card}>
                    ...
                    {campaign.productImageUrl && <Image source={{ uri: `${apiUrl}${campaign.productImageUrl}` }} accessibilityLabel={campaign.productName} style={{ width: "100%", height: 200, borderRadius: 16 }} resizeMode="cover" />}
                    <Text style={styles.body}>{campaign.description}</Text>
                    <Text style={styles.price}>
                      {campaign.compensation.type === "paid"
                        ? `${(campaign.compensation.amountPerReelMinor / 100).toFixed(2)} ${campaign.currency} pro Reel`
            ...
            <View style={styles.empty}>
              ...
              {authenticated && (
                <Pressable
                  accessibilityRole="button"
                  onPress={() => router.push("/brand-campaigns")}
                  style={styles.button}
                >
...
const styles = StyleSheet.create({
  ...
  roleText: { fontWeight: "600", color: "#526256" },
  roleTextActive: { color: "#FFFFFF" },
  hero: { paddingVertical: 14, gap: 18 },
  eyebrow: {
    fontSize: 10,
    letterSpacing: 2,
    color: "#527362",
  ...
  sectionTitle: { fontSize: 20, color: "#203B30", fontWeight: "600" },
  ...
  filterActive: { backgroundColor: "#203B30", borderColor: "#203B30" },
  filterText: { color: "#526256", fontWeight: "500" },
  ...
  emptySymbol: { fontSize: 38, color: "#78916B" },
  emptyTitle: {
    fontSize: 22,
    color: "#203B30",
    fontWeight: "600",
  ...
  body: { color: "#647067", fontSize: 14, lineHeight: 23 },
  button: {
    backgroundColor: "#36584A",
  ...
  price: { color: "#36584A", fontSize: 18, fontWeight: "700" },
  note: { fontSize: 12, lineHeight: 19, color: "#73766D", marginTop: 6 },
  how: { gap: 22, paddingTop: 12 },
  step: { flexDirection: "row", gap: 18 },
  stepNumber: {
    color: "#779166",
    fontSize: 13,
    fontWeight: "700",
  ...
  stepTitle: { fontSize: 16, fontWeight: "600", color: "#203B30" },
  footer: {
    color: "#6F796D",
    fontSize: 9,
    letterSpacing: 1.3,
