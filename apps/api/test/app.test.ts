...
test('liveness survives a database outage, readiness and discovery report unavailable without leaking secrets', async (t) => {
  const fail = async (): Promise<never> => {
    throw new Error('postgresql://private:password@server/db');
  };
  const app = buildApp({
    database: makeDatabase({ ping: fail, listCampaigns: fail }),
    origins: [],
  });
  t.after(() => app.close());
...
test('invalid filters never reach the database; valid filters are forwarded', async (t) => {
  ...
  t.after(() => app.close());
  assert.equal(
    (await app.inject('/v1/campaigns?dealType=invalid')).statusCode,
    400
  );
  assert.equal(
    (await app.inject('/v1/campaigns?unexpected=true')).statusCode,
    400
  );
