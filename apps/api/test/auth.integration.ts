...
test('real PostgreSQL: auth lifecycle, isolated profiles, CSRF, role protection and session revocation', async t => {
  ...
  const app = buildApp({ database: createDatabase(config.DATABASE_URL,pool), origins:[origin], auth, profiles:createProfileStore(pool), authBaseUrl:config.AUTH_BASE_URL });
