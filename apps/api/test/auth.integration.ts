...
import { deleteCampaignImage } from '../src/modules/campaigns/images.js';
...
test('real PostgreSQL: auth lifecycle, isolated profiles, CSRF, role protection and session revocation', async (t) => {
  ...
  const me = (cookie?: string) =>
    app.inject({ url: '/v1/me', headers: cookie ? { cookie } : {} });
  const save = (
    cookie: string,
  ...
  const creatorEmail = 'creator@example.test',
    brandEmail = 'brand@example.test';
  async function signUp(email: string) {
    const response = await post('sign-up/email', {
      name: 'Test account',
      email,
      password,
      callbackURL: `${config.AUTH_BASE_URL}/auth/verified`,
    });
    assert.equal(response.statusCode, 200, response.body);
    const message = [...mail]
      .reverse()
      .find((item) => item.to === email && item.subject.includes('bestätigen'));
  ...
  await t.test(
    'unverified account cannot log in or read private data',
    async () => {
      assert.equal(
        (await post('sign-in/email', { email: creatorEmail, password }))
          .statusCode,
        403
      );
  ...
  const creatorLogin = await post('sign-in/email', {
    email: creatorEmail,
    password,
  });
  ...
  const brandLink = await signUp(brandEmail);
  await verify(brandLink);
  const brandLogin = await post('sign-in/email', {
    email: brandEmail,
    password,
  });
  ...
  await t.test(
    'onboarding is idempotent and roles own distinct profiles',
    async () => {
      assert.equal((await me(creatorCookie)).json().profile, null);
      const results = await Promise.all([
        save(creatorCookie, creator),
        save(creatorCookie, creator),
      ]);
      ...
      assert.equal(
        (await pool.query('SELECT count(*) FROM profiles')).rows[0].count,
        '2'
      );
    }
  );
  await t.test(
    'ownership cannot be injected, roles cannot be switched, and writes require a trusted origin',
    async () => {
      assert.equal(
        (await save(brandCookie, { ...brand, id: creatorId })).statusCode,
        400
      );
      assert.equal((await save(creatorCookie, brand)).statusCode, 409);
      assert.equal(
        (await save(creatorCookie, creator, 'https://evil.example')).statusCode,
        403
      );
      assert.equal(
        (await save(creatorCookie, { ...creator, dealPreferences: [] }))
          .statusCode,
        400
      );
      assert.equal(
        (
          await save(creatorCookie, {
            ...creator,
            portfolioUrls: ['https://evil.example/reel/abc/'],
          })
        ).statusCode,
        400
      );
      assert.equal(
        (await save(brandCookie, { ...brand, brandName: 'Updated Brand' }))
          .statusCode,
        200
      );
      assert.equal(
        (await me(creatorCookie)).json().profile.details.displayName,
        'Creator'
      );
      assert.equal(
        (
          await app.inject({
            url: `/v1/profiles/${creatorId}`,
  ...
  await t.test(
    'campaign ownership, bodyless publishing, upload validation and feed lifecycle',
    async () => {
      const input = {
        title: 'Test campaign',
        productName: 'Product',
        description: 'Create a Reel',
        compensation: { type: 'barter', productValueMinor: 1200 },
        currency: 'EUR',
        reelCount: 1,
      ...
      assert.equal(
        (await request('POST', '/v1/brand/campaigns', input, creatorCookie))
          .statusCode,
        403
      );
      assert.equal(
        (
          await request('POST', '/v1/brand/campaigns', {
            ...input,
            brandId: randomUUID(),
          })
        ).statusCode,
      ...
      const id = created.json().id,
        url = `/v1/brand/campaigns/${id}`;
      await verify(await signUp('other-brand@example.test'));
      const otherCookie = cookieOf(
        await post('sign-in/email', {
          email: 'other-brand@example.test',
          password,
        })
      );
      assert.equal(
        (await save(otherCookie, { ...brand, brandName: 'Other Brand' }))
          .statusCode,
        200
      );
      for (const action of ['/publish', '/close', '/image']) {
        const payload =
          action === '/image'
            ...
            : undefined;
        assert.equal(
          (await request('POST', url + action, payload, otherCookie))
            .statusCode,
          404
        );
      }
      assert.equal(
        (await request('PUT', url, input, otherCookie)).statusCode,
        404
      );

      assert.equal(
        (await app.inject('/v1/campaigns')).json().campaigns.length,
        0
      );
      let invoked = false;
      await assert.rejects(
        createCampaignStore(pool).setProductImage(
          (await me(brandCookie)).json().user.id,
          randomUUID(),
          async () => {
            invoked = true;
            return '/invalid';
      ...
      assert.equal(
        invoked,
        false,
        'unauthorized/missing campaign must not write a file'
      );
      assert.equal(
        (
          await request('POST', `${url}/image`, {
            mimeType: 'image/png',
      ...
      const bytes = await sharp({
        create: { width: 2000, height: 10, channels: 3, background: '#123456' },
      })
        .png()
        .toBuffer();
      const uploaded = await request('POST', `${url}/image`, {
        mimeType: 'image/png',
      ...
      try {
        ...
        await deleteCampaignImage(imageUrl);
      }
    }
  );
  ...
  await t.test(
    'password reset is one-time, rejects bad tokens, and revokes old sessions',
    async () => {
      ...
      const message = [...mail]
        ...
        .find(
          (item) =>
            item.to === creatorEmail && item.subject.includes('zurücksetzen')
        )!;
      ...
      const token = new URL(String(redirect.headers.location)).searchParams.get(
        'token'
      );
      assert.ok(token);
      const newPassword = 'A-new-local-password-456';
      assert.equal(
        (await post('reset-password', { token: 'invalid', newPassword }))
          .statusCode,
        400
      );
      assert.equal(
        (await post('reset-password', { token, newPassword })).statusCode,
        200
      );
      assert.equal(
        (await post('reset-password', { token, newPassword })).statusCode,
