...
export function registerAuthPages(app: FastifyInstance) {
  app.addHook('onSend', async (request, reply, payload) => {
    if (request.url.startsWith('/auth/') || request.url.startsWith('/api/auth/') || request.url.startsWith('/v1/me')) {
      reply.header('Cache-Control', 'no-store');
      reply.header('Referrer-Policy', 'no-referrer');
      reply.header('X-Content-Type-Options', 'nosniff');
      reply.header('Content-Security-Policy', "default-src 'none'; script-src 'self'; style-src 'unsafe-inline'; connect-src 'self'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'");
    }
    return payload;
  });
  app.get('/auth/verified', async (request, reply) => {
    const hasError = new URL(request.url, 'http://local').searchParams.has('error');
    return reply.type('text/html').send(page('E-Mail bestätigen', hasError
  ...
  app.get('/auth/reset-password', async (_request, reply) => reply.type('text/html').send(page('Passwort zurücksetzen', `
    <h1>Dein neues Passwort.</h1><p>Wähle mindestens 10 Zeichen. Danach meldest du dich in der App erneut an.</p>
    <form id="reset"><label for="password">Neues Passwort</label><input id="password" type="password" minlength="10" maxlength="128" autocomplete="new-password" required>
    <label for="confirm">Passwort wiederholen</label><input id="confirm" type="password" minlength="10" maxlength="128" autocomplete="new-password" required>
  ...
  app.get('/auth/reset.js', async (_request, reply) => reply.type('application/javascript').send(`
