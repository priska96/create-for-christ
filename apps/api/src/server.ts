...
const pool = new pg.Pool({ connectionString: config.DATABASE_URL, max: 10, connectionTimeoutMillis: 3000 });
...
const app = buildApp({
  ...
  logger: true, auth, profiles: createProfileStore(pool), authBaseUrl: config.AUTH_BASE_URL,
  beforeClose: () => mailer.close(),
});
for (const signal of ['SIGINT','SIGTERM'] as const) {
  process.once(signal, () => { void app.close().catch(() => { process.exitCode = 1; }); });
}
try { await app.listen({host:config.HOST,port:config.PORT}); mailer.start(); }
